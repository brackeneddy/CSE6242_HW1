...
class Graph:
    ...
    def max_degree_nodes(self) -> dict:
        ...
        max_degree_nodes = Counter(node for edge in self.edges for node in edge)

        max_degree = max(max_degree_nodes.values())
        max_nodes = {node: degree for node, degree in max_degree_nodes.items() if degree == max_degree}
...
class  TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list=None) -> list:
        ...
        if response.status == 200:
            cast = json.loads(response.read().decode('UTF-8'))['cast']
            if exclude_ids is not None:
                cast = [i for i in cast if i['id'] not in exclude_ids]
            if limit is not None:
                cast = [i for i in cast if i['order'] < limit]
            cast = [{**c, 'name': c['name'].replace(',', '')} for c in cast if 'name' in c and isinstance(c['name'], str)]
            return cast
        else:
    ...
    def get_movie_credits_for_person(self, person_id:str, vote_avg_threshold:float=None)->list:
        ...
        response = conn.getresponse()
        if response.status == 200:
            raw_credits = json.loads(response.read().decode('UTF-8'))['cast']
            credits = [
                {"id": idx["id"], "title": idx["title"], "vote_avg": idx["vote_average"]}
                for idx in raw_credits
                if vote_avg_threshold is None or idx["vote_average"] >= vote_avg_threshold
            ]
            print(credits)
            return credits
        else:
...
if __name__ == "__main__":
    ...
    tmdb_api_utils = TMDBAPIUtils(api_key='74be3b4aec3845977cebc16cfa45aa33')
    ...
    for x in range(2):
        if x == 0:
            ...
            current_nodes = [x for x in graph.nodes if x not in current_nodes]
    ...
    print(str(graph.total_edges()))
    print(str(graph.total_nodes()))
